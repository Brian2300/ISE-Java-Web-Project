	...
	//	}
	    ...
	    public Professor retrieveProfessor(String emailID, String password) {
	        ...
	        try {
	            ...
	            sql = "select smu_email, password from " + TBLNAME + " where smu_email_id = ? and password = binary?";
	    ...
	    public boolean CheckUniqueEmail(String email) {
	        ...
	        try {
	            ...
	            sql = "select smu_email, password from " + TBLNAME + " where smu_email = ? ";
	            stmt = conn.prepareStatement(sql);
	    ...
	    public String registerProfessor(String email,String password){
		    ...
		    try {
		    	conn = ConnectionManager.getConnection();
		    	sql = "insert into professor (smu_email, smu_email_id,password) values (?,?,?)";
		        stmt = conn.prepareStatement(sql);
		        stmt.setString(1, email);
		        stmt.setString(2, email.substring(0,email.indexOf("@")));
