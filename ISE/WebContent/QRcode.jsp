	...
	<%
	   ...
	   // String passcode = (String) request.getAttribute("passcode");
		...
		if (!b) { 
			...
			out.println("<h4 class='card-title'>Passcode: /TakeAttendance "+ passcode+ "  generated at  "+ time+"</h4>");
			//out.println("<img src='images/"+passcode+".png' class='rounded mx-auto d-block'alt='QR Code' width='25%'>");
			out.println("<img src='/ISE/RetrieveImg' class='rounded mx-auto d-block'alt='QR Code' width='25%'>");
			out.println("</div>");
			out.println("</div>");
		}
		...
		out.println("<button type='submit' class='btn btn-primary' name='stop' value='stop'>Stop</button>");
		out.println("</div>");
	    out.println("</div>");
		out.println("</form>");
		out.println("</div>");
